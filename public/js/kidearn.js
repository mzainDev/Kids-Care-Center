(function ($) {
  ...
  if (kidearnTiltElm.length) {
    kidearnTiltElm.each(function () {
      ...
      // Safely get tilt options
      let options = $(this).attr("data-tilt-options"); // safer than `.data()` if issue exists
      let tiltOptions = {};
  
      try {
        tiltOptions = typeof options === "string" ? JSON.parse(options) : {};
      } catch (e) {
        console.warn("Tilt options JSON parse failed:", options);
      }
